...
local Selects = {
    ...
    {Title = "Одежда", Func = function(luaMenu,pp) hg.CreateApperanceMenu(pp) end},
	{Title = "Правила", Func = function(luaMenu, pp)
    local rulesPanel = vgui.Create("DPanel", pp)
    rulesPanel:Dock(FILL)
    rulesPanel.Paint = function(self, w, h)
        draw.RoundedBox(0, 0, 0, w, h, Color(20, 20, 20, 220))
    end

    local scroll = vgui.Create("DScrollPanel", rulesPanel)
    scroll:Dock(FILL)
    scroll:DockMargin(20, 20, 20, 20)

    local txt = vgui.Create("DLabel", scroll)
    txt:SetText(
        "1. Не читерить\n" ..
        "2. Не использовать баги\n" ..
        "3. Уважать игроков\n" ..
        "4. Не мешать другим во время раунда"
    )
    txt:SetTextColor(color_white)
    txt:SetFont("ZCity_Small")
    txt:SetWrap(true)
...
local splasheh = {
    ...
    'Я ТВОЙ РОТ ЕБАЛ',
    'ОТСОСИТЕ',
    'ОТ-СИТИ УЖАСНЫЙ СЕРВЕР (мой такой же)',
